...
const mapCatalogNameToCrseInfo = require("../course/get_course_info");
...
const addStudentSurveyResponse = async (
	...
	const coursesInfo = await Promise.all(
		courseCatalogNames.map((name) => mapCatalogNameToCrseInfo(name))
	);
	...
	const studentUpdate = studentRef
		...
		.catch((err) => {
			console.log(err);
			const e = new Error(
				`Error in processing studentUpdate for ${email}`
			);
	...
	const courseUpdates = courseCatalogNames.map((courseCatalogName, index) =>
		courseRef
			.doc(coursesInfo[index].courseId.toString()) // We want the courseID to allow for crosslisting
			.get()
			.then((snapshot) => {
